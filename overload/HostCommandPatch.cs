	...
	public static class HostCommandPatch
	{
							...
							else
							{
								...
								else
								{
									...
									else
									{
										...
										if (flag14)
										{
											DestroyableSingleton<HudManager>.Instance.Chat.AddChat(PlayerControl.LocalPlayer, flag13 ? "Usage: s! ban <player_id> <reason>" : "Usage: s! kick <player_id> <reason>", true);
											result = false;
										}
										else
										{
		...
		public class RolesUI : MonoBehaviour
		{
			...
			private void DrawWindowContents(int id)
			{
				...
				if (flag2)
				{
					Rect rect = new Rect(0f, 0f, this.windowRect.width - 32f - 10f, this.windowRect.height);
					GUI.DragWindow(rect);
				}
			}
			...
			private void DrawCloseButton()
			{
				...
				GUI.Label(this.closeButtonRect, "?", guistyle2);
			...
			private void HandleResize()
			{
				...
				if (flag3)
				{
					...
					bool flag5 = current.type == 1 && current.button == 0;
					if (flag5)
					{
			...
			static RolesUI()
			{
				HostCommandPatch.RolesUI.availableRoles = new RoleTypes[] { RoleTypes.Crewmate, RoleTypes.Impostor, RoleTypes.Scientist, RoleTypes.Engineer, RoleTypes.GuardianAngel, RoleTypes.Shapeshifter, RoleTypes.Crewmate, RoleTypes.Crewmate, RoleTypes.Impostor, RoleTypes.Scientist, RoleTypes.Engineer };
